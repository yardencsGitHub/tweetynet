...
def main(birdsongrec_root, data_root):
    ...
    for bird in pbar:
        ...
        keys = []  # "keys" = list of filenames that SongAnnotationGUI matches to annotation "elements"
        # "elements" = Matlab structs each with fields filenum, segFileStartTimes, segFileEndTimes and segType
        elements = []

        for filenum, annot in enumerate(annots):
            keys.append(annot.audio_path.name)
            labels = np.array(
