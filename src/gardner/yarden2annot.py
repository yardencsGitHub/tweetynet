...
def yarden2annot(annot_file,
    ...
    array_paths = annot_mat[fname_key]
    annotations = annot_mat[annot_key]
    if len(array_paths) != len(annotations):
    ...
    for array_path, annotation in zip(array_paths, annotations):
        audio_path = _recursive_stem(array_path)  # used below when creating Annotation
